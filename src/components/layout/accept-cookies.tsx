...
function CookieConsent() {
  ...
  return (
    <div className={cn(
      ...
      "bg-[#FF3F1A] text-[#F0EDE8] md:text-sm xs:text-[0.5625rem] text-[0.4375rem]",
      "transition-transform duration-300 ease-in-out", isVisible ? "translate-y-0" : "translate-y-full",
    )}>
      <div className="flex justify-between items-center px-[8.2vw] py-4">
        ...
        <button onClick={handleAccept} className="ml-5 text-[#F0EDE8] underline">Accept Cookies</button>
      </div>
    </div>
  );
}
