...
export default function Header({ className }: { className?: string }) {
  ...
  return (
    <>
      ...
      <header
        className={cn("w-full text-white xs:relative fixed z-100", className)}
      >
        <div className={cn("mx-auto px-5 flex items-center")}>
          {/* Logo */}
          <Link href="/" className="flex-shrink-0 flex items-center group">
            <FunctionLogo className="origin-left xs:scale-100 scale-119 fill-white group-hover:fill-[#FF3F1A] transition-colors ease-in-out duration-300" />
          </Link>
          <div className="grow-1"></div>
          {/* Desktop Navigation */}
          <nav className="hidden xs:flex shrink-0 items-baseline md:space-x-15 space-x-7.5 flex-nowrap">
            {menuItems.map((item) => (
              <HeaderItem
                className="shrink-0"
                key={item.label}
            ...
            {/* Get Price Button */}
            <button
              className="hidden xs:block bg-[#F0EDE8] text-[#151516] hover:bg-[#FF3F1A] transition-colors duration-200 py-0.5 rounded-full leading-8.5 px-5 text-sm font-medium text-nowrap"
              onClick={openModal}
            >
              Get Price
            </button>
